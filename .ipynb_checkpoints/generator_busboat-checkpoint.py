...
from utils.image_tweak import apply_random_scale_and_crop, random_distort_image, random_flip, correct_bounding_boxes, random_paste
import copy
...
class BatchGenerator(Sequence):
    ...
    def _aug_image(self, instance, net_h, net_w):
        ...
        #------------Nonlinear augmentation--------------------          
        if self.nl_mixup > 0.0:
            
            if np.random.uniform() < 0.5: #BOAT
                name = 'boat'
                number = np.random.randint(low=0, high=398)
                chosen = './cropped_data/cropped_boat/boat_' + str(number) + '.jpg'
            else: #BUS
                name = 'bus'
                number = np.random.randint(low=0, high=272)
                chosen = './cropped_data/cropped_bus/bus_' + str(number) + '.jpg'
            alien = cv2.imread(chosen)
            
            
            mixup_rate = self.nl_mixup
            if np.random.uniform() < mixup_rate:
                image, bbox = random_paste(image, alien)
        ...
        all_objs = correct_bounding_boxes(boxes, new_w, new_h, net_w, net_h, dx, dy, flip, image_w, image_h)
